...
class Model(object):
	...
	def generator(self, h, w, c, filters):
		...
		h = tk.Sequential([x,
			...
			Conv2d(filters, 3, 1), BN(), Lrelu(),
			Conv2d(filters, 3, 1), BN(), Lrelu(),
			Conv2d(self.args.img_nc, 3, 1, activation='tanh')]).output + y

		return tk.Model([x, y], h)
	...
	@tf.function
	def train_step(self, scale, rec_prev):
		img, noise_weight = self.imgs[scale], self.noise_weights[scale]
		z = tf.random.normal(img.shape)
		...
		for j in range(self.args.D_step):
			...
			vars_d = self.Ds[scale].trainable_variables
		...
		for j in range(self.args.G_step):
			...
			vars_g = self.Gs[scale].trainable_variables
			self.optimizer_g.apply_gradients(zip(tape_g.gradient(loss_g, vars_g), vars_g))

		return {'loss_g_adv': loss_g_adv, 'loss_g_rec': loss_g_rec, 'loss_d_adv': loss_d_adv, 'loss_d_gp': loss_d_gp, 'fake': fake, 'rec': rec}
	...
	def train(self):
		...
		for scale in range(self.num_scale):
			...
			for i in range(self.args.iteration):
				item = self.train_step(scale, rec_prev)
				print('scale: [%d/%d] iter: [%4d/%4d] time: %.2f loss_g_adv: %.4f, loss_g_rec: %.4f, loss_d_adv: %.4f, loss_d_gp: %.4f' % (
					scale, self.num_scale, i, self.args.iteration, time.time() - start_time, 
					item['loss_g_adv'].numpy(), item['loss_g_rec'].numpy(), item['loss_d_adv'].numpy(), item['loss_d_gp'].numpy()))

				if (i + 1) % self.args.sample_freq == 0:
					imsave(os.path.join(self.args.sample_dir, f'fake_{scale}_{i + 1}.jpg'), imdenorm(item['fake'].numpy()[0]))
					imsave(os.path.join(self.args.sample_dir, f'rec_{scale}_{i + 1}.jpg'), imdenorm(item['rec'].numpy()[0]))
